<x-layouts.app>
    <x-page-header title="Laporan" subtitle="Ringkasan penjualan dan riwayat transaksi" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
            ...
            <p class="text-2xl font-bold text-emerald-600">Rp {{ number_format($transactions->sum('total'), 2) }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
            <p class="text-xs text-slate-400 mb-1">Rata-rata Transaksi</p>
            <p class="text-2xl font-bold text-slate-900">
                Rp {{ $transactions->count() ? number_format($transactions->avg('total'), 2) : '0.00' }}
            </p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-semibold text-emerald-600">Rp {{ number_format($tx->total, 2) }}</span>
                        </td>
