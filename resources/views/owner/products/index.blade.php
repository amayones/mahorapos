<x-layouts.app>
    <x-page-header title="Products" subtitle="Manage your product catalogue" action-label="Add Product" />

    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-slate-700">All Products</h2>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-slate-700 font-medium">RM {{ number_format($product->price, 2) }}</td>
                        <td class="px-6 py-4">
                            @if($product->stock === 0)
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-600">Out of stock</span>
                            @elseif($product->stock < 10)
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">Low ({{ $product->stock }})</span>
                            @else
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">{{ $product->stock }} in stock</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <button class="text-xs text-indigo-500 hover:text-indigo-700 font-medium transition-colors">Edit</button>
                                <button class="text-xs text-red-400 hover:text-red-600 font-medium transition-colors">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <p class="text-slate-400 text-sm">No products yet. Add your first product.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
